...
class Node:
    ...
    # invert (abstract)
    def invert(self):
        raise NotImplementedError("invert must be implemented by subclasses")
...
class Leaf(Node):
    ...
    # invert (base case)
    def invert(self):
        # Leaf has no children—nothing to swap
        return self   
...
class Internal(Node):
    ...
    def invert(self):
        ...
        new_left = self.right.invert()
        new_right = self.left.invert()

        # return new node with swapped children
        return Internal(self.node_data, new_left, new_right)
...
def main():
    ...
    print("\nInverted:")
    inverted = root.invert()
    print(inverted)                 # <5, <7, 9, 6>, 3>
